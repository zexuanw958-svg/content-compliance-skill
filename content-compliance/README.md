...
- 中文输出：默认生成简体中文检测报告，只有规则 ID、来源 ID、`Status` 等必要机器标识保留英文。
- 选题检测：在写稿前判断选题是否值得继续做。
- 成稿检测：检查标题、口播、正文、字幕、画面、评论区、私信计划。
- 可视化检测：输出 10 格风险条和分项安全诊断分。
- 投放语境检测：区分普通发布和 DOU+、巨量引擎、薯条、蒲公英、聚光等商业化场景。
- 官方来源约束：规则卡优先追溯公开官方来源，并把 `Status: active` 的确认风险和 `Status: needs_review` 的待复核提示分开。
- 官方来源复核：用 `references/rule-refresh.md` 定期检查官方来源是否有可见更新，再同步规则卡、示例和测试。

## 官方来源边界

这个 Skill 采用官方来源优先，而不是把网上通用说法直接当规则。确认风险必须追溯到 `references/sources.md` 里的官方来源；解释不够清楚的来源会保留为 `Status: needs_review`。

这不是自动实时同步所有平台规则，也不是官方审核工具。检测结果只作为发布前参考，最终仍以平台官方审核、账号状态、内容上下文和实际执行口径为准。
...
- `总风险分`：1-10，越高越危险。
- `风险条`：固定 10 格；填充格数等于总风险分，填充颜色由风险档位统一决定。
